...
Consider the following example specificallyC++ code:

```cpp
// This code illustrates how various data types can be intermixed in C++      
// Add operand takes all input and output data types and converts as needed   
// Integer output from floating point inputs are rounded to integer          
...
k = i + J;    // k = 13    32 bit signed and 16 bit unsigned mixed    
z = x + y;    // z = 28.3  32 bit float and 64 bit double mixed       
z = x + j;    // z = 30.5  32 bit float and 16 bit unsigned int mixed 
j = x + j;    // k = 30    32 bit float rounded to 16 bit integer     
z = j / i;    // z = 3.33  floating point result to integer divide 
k = j / i;    // k = 3     result rounded to integer 

```



These few examples illustrate the point that in the C++ language there is a limited number of arithmetic operators ( + - * / % ) which work with any data type or combination of data types. This is not the way in most machine language codes. Most machine languages have separate instructions for each data type. Furthermore, the two input operands and the output operands must be the same type. Typical RISC and CISC ISAs define instructions such as:
...
//  The source-1, source-2 and destination registers must be the same type and length 
	...
	addib	id,is1,is2      // Add Integer Byte (8 bits)   
    addibu  id,is1,is2    	// Add Integer Byte unsigned
    addih 	id,is1,is2      // Add Integer Half (16 bits)
    add1hu	id,is1,is2  	// Add Integer Half unsigned
